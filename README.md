...
この仕組みを設けること自体には私も何ら異論はないのですが、
個人で辞書を作成する場合や各ディストリビューションで配布を行う際に辞書をMergeする場合に、
一律的な基準で単語のフィルタリングを行うというのは問題が起こるかも知れないな。と言う懸念を感じてもいます。

特に、ディストリビューション…DebianだったりUbuntuだったりFedoraだったりFreeBSDだったりと…での配布を考えると、
それぞれの行動規範や政治的基準と言うものがバラバラであり、それ自体はFOSSの最大の強みだとは思うのですが、
Upstreamに置かれている、辞書作成側での禁止ワードがUpstreamの設定のままで機械的に反映されてしまうことが
却って、それらのポリシーと衝突しかねないという問題を含むことになります。
勿論、AI・人工知能/LLMの使用に関する各ディストリビューションのポリシーの違いという問題も潜在的な障害というかリスク要因になりつつありますし。

従って、フィルタリングを回避したり、独自のフィルタリングリストと差し替えて辞書データを作成できるようにしたり、
そのようなノウハウ自体のドキュメント化をしておくことは、
今後各ディストリビューションの公式／非公式パッケージとして配布する上では避けて通れない道だと考えています。

又、Mozcも3.x系になって、外部辞書の追加がより容易になったようです(これから試験していきますが)。
その辺りのノウハウ文書も追々入れていこうと思います。
